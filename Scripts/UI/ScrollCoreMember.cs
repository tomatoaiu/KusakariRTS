...
public class ScrollCoreMember : MonoBehaviour {

	[SerializeField]
	GameObject dropImage;  //ボタンプレハブ
...
//	ToNextScene toNextScene;
	...
	void Start ()
	{
		...
		float btnWidth = dropImage.GetComponent<LayoutElement>().preferredWidth;
		...
		for (int i = 0; i < BUTTON_COUNT; i++)
		{
			int no = i;
			//ボタン生成
			GameObject btn = (GameObject)Instantiate(dropImage);
			//ボタンをContentの子に設定
